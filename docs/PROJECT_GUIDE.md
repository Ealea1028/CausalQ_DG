...
| Phase 14 | Learned-null 干预基线 | 修复后的隔离 smoke 已通过；等待 seed-0 40k 正式运行 |
...
Phase 12 已固定 `R=2`。Phase 13 最终选择 Static：其三 seed 为 `0.652849 ± 0.015138`，且 seed-0 Bidirectional 只有 `0.630394`。随后的零消融定位审计在三个 Static seed 上得到 `2.913 ± 0.619` 的类内/类外绝对效应比，平均 `86.48%` 的 GT-present 类别图在对应区域内效应更强，证明现有 Query effect 具有可复现的区域语义。Phase 14 因此只加入所有类别共享的 R=2 learned-null bank，并用 stop-gradient factual-query centroid 校准。修复初始化顺序后的 `d85db4e` smoke 已通过：500 条记录连续有限、目标重构误差 `1.790e-7`，null loss 下降约 `84.8%`。下一步只运行 seed-0 40k learned-null，不同时加入 effect invariance、sufficiency 或 specificity，也不自动启动重复种子。
