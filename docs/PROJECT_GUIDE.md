...
| Phase 14 | Learned-null 干预基线 | seed-0 40k 已完成；等待 zero/learned-null 配对效应定位分析 |
...
Phase 12 已固定 `R=2`。Phase 13 最终选择 Static：其三 seed 为 `0.652849 ± 0.015138`，且 seed-0 Bidirectional 只有 `0.630394`。随后的零消融定位审计在三个 Static seed 上得到 `2.913 ± 0.619` 的类内/类外绝对效应比，平均 `86.48%` 的 GT-present 类别图在对应区域内效应更强。Phase 14 的 learned-null seed-0 40k 已完成：最终 mIoU `0.655364`，比配对 Static 高 `0.7968` 个百分点，null loss 收敛至 `2.614e-13`。下一步只读比较同一 final checkpoint 的 zero 与 learned-null effect 定位和背景幅度，不同时加入 effect invariance、sufficiency 或 specificity，也不自动启动重复种子。
